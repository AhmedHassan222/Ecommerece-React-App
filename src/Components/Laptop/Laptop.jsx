...
export default function Laptop() {
    let { computers, addToCart, addToFavarite, getProducts, itemsInCart, favariteArr } = useContext(favariteContext)
    ...
    return <>
        <Helmet>
            <title>Laptop - eMarket  </title>
        </Helmet>
        {computers.length <= 0 ? <div className='vh-100 d-flex justify-content-center  p-5 w-100 align-self-center'>
            ...
            {isLoad == false ? <div>
                <div className="row g-3">
                    {computers.slice(0, 8).map((item, index) => <div key={index} className={`  col-sm-12 col-md-4 col-lg-3`}>
                        <div className={`bg-white p-3 ${style.content}`}>
                            <img className="w-100" src={item.img} alt="" />

                            <div className={`${style.caption}`}>
                                <Link to={'/productdetails/' + 'computers/' + item.id} className={`${style.box} p-3`}>
                ...
                <div className="row g-3">
                    {computers.map((item, index) => <div key={index} className={` col-sm-12 col-md-4 col-lg-3`}>
                        <div className={`bg-white p-3 ${style.content}`}>
                            <img className="w-100" src={item.img} alt="" />
