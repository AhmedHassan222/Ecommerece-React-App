...
export default function Headphones() {
    let { headphones, addToCart, addToFavarite, getProducts, itemsInCart , favariteArr } = useContext(favariteContext)
    ...
    return <>
        <Helmet>
            <title>Headphones - eMarket  </title>
        </Helmet>
        {headphones.length <= 0 ? <div className='vh-100 d-flex justify-content-center  p-5 w-100 align-self-center'>
            ...
            {isLoad == false ? <div>
                <div className="row g-3">
                    {headphones.slice(0, 8).map((item, index) => <div key={index} className={` col-xsm-6 col-sm-12 col-md-4 col-lg-3`}>
                        <div className={`${style.content} bg-white  p-3`}>
                            <img className="w-100" src={item.img} alt="" />
                            <div className={`${style.caption}`}>
                                <Link to={'/productdetails/' + 'headphones/' + item.id} className={`${style.box} p-3`}>
                ...
                <div className="row g-3">
                    {headphones.map((item, index) => <div key={index} className={` col-sm-12 col-md-4 col-lg-3`}>
                        <div className={`${style.content} bg-white  p-3`}>
                            <img className="w-100" src={item.img} alt="" />
                            <div className={`${style.caption}`}>
                                <p className={`${style.textNameContent}`}>{item.name}</p>
